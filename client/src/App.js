...
function App() {
  ...
  const fetchVehicles = async () => {
    try {
      const response = await axios.get("https://vehicle-task.onrender.com/api/vehicles");
  ...
  const addVehicle = async () => {
    try {
      await axios.post("https://vehicle-task.onrender.com/api/vehicles", newVehicle);
      setNewVehicle({
        chassisNumber: "",
        registrationNumber: "",
        lastLocation: { lat: 0, long: 0 },
  ...
  const updateVehicle = async () => {
    try {
      await axios.put(
        `https://vehicle-task.onrender.com/api/vehicles/${editingVehicle._id}`,
        newVehicle
      );
  ...
  const deleteVehicle = async (id) => {
    try {
      await axios.delete(`https://vehicle-task.onrender.com/api/vehicles/${id}`);
      fetchVehicles();
    } catch (error) {
